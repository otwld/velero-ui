<template>
  ...
  <div
    ...
    role="tooltip"
  >
    {{ t('form.tooltip.labels', { type: t('global.snapshotLocation', 1) }) }}
    <div class="tooltip-arrow" data-popper-arrow></div>
  </div>
</template>
...
const removeConfig = (keyConfig: string) => {
  delete currentForm.value.configuration[keyConfig];
};
...
const removeLabel = (keyConfig: string) => {
  delete currentForm.value.labels[keyConfig];
};
