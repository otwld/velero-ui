  ...
  >
    ...
    >
      <div class="flex items-center justify-between mb-1">
        ...
        />
        <span class="text-xs font-medium text-gray-900 dark:text-white">{{ backup.status.progress.itemsBackedUp || '0' }} /
    ...
    <div
      v-if="backup?.status?.failureReason"
      class="mt-4 flex flex-col"
    >
      <span class="text-base font-medium text-gray-900 dark:text-white">{{
        t('resource.status.reason')
      }}</span>
      <i class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ backup.status.failureReason }}</i>
    </div>
    <div
      v-if="backup?.status?.validationErrors"
      class="mt-4 flex flex-col"
    >
      <span class="text-base font-medium text-gray-900 dark:text-white">{{
